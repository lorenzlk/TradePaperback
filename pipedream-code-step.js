...
export default defineComponent({
  async run({ steps, $ }) {
    // Get body from HTTP trigger
    // This workflow handles BOTH barcode scans and cover scans
    const body = steps.trigger?.event?.body;
    
    console.log('Request received. Body type:', typeof body);
    console.log('Body keys:', body && typeof body === 'object' ? Object.keys(body) : 'N/A');
    
    // Determine request type: barcode scan (has 'upc') or cover scan (has 'image')
    const isBarcodeScan = body && typeof body === 'object' && body.upc;
    const isCoverScan = body && typeof body === 'object' && body.image;
    
    // Handle barcode scan - pass through to next step (Google Sheets)
    if (isBarcodeScan) {
      console.log('📷 Barcode scan detected. UPC:', body.upc);
      return {
        success: true,
        type: 'barcode_scan',
        data: body, // Pass through entire payload for Google Sheets step
        message: 'Barcode scan received, forwarding to Google Sheets'
      };
    }
    
    // Handle cover scan - process with Vision API
    if (isCoverScan) {
      console.log('📸 Cover scan detected. Processing with Vision API...');
      
      // Extract image from body
      let image = body.image || body.data?.image || body.content || body.payload;
      
      if (!image) {
        return {
          success: false,
          error: 'No image provided in cover scan request',
          debug: {
    ...
    // If neither barcode nor cover scan, return error
    return {
      success: false,
      error: 'Invalid request. Expected either "upc" (barcode scan) or "image" (cover scan) in request body.',
      debug: {
